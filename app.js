...
async function fetchSeriesImage(title){
 const q=encodeURIComponent(normalizeShowQuery(title));
 try{
  const r=await fetch(`https://api.tvmaze.com/singlesearch/shows?q=${q}`,{headers:{Accept:"application/json"}});
  if(!r.ok)return null;
...
function renderApp(){
 ...
 <section class="hero" id="hero"></section><main class="container"><div id="catalog"></div></main><footer class="footer">StreamDrive • catálogo local para GitHub Pages</footer><div id="modal" class="modal hidden"></div>`;
 renderCategoryMenus();
